...
def create_age_group(age_group: AgeGroupSchema, username: str = Depends(get_current_username)):
    return create_age_group_service(age_group)

@router.delete("/{age_group_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_age_group(age_group_id: str, username: str = Depends(get_current_username)):
    delete_age_group_service(age_group_id)
    return None

@router.get("/", response_model=List[AgeGroupSchema], status_code=status.HTTP_200_OK)
def list_age_groups(username: str = Depends(get_current_username)):
